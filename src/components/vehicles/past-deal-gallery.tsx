...
/**
 * Past-deal gallery. A trimmed `VehicleGallery` - same large photo, thumbnail
 * strip and fullscreen viewer with arrow keys, Escape and touch swipe - minus
 * the category chips, since a past deal's photos aren't tagged exterior /
 * interior / dashboard.
 */
export function PastDealGallery({
  images,
  title,
}: {
  images: Pick<PastDealImageRow, 'id' | 'url' | 'alt_text'>[]
  title: string
}) {
  ...
  const active = images[Math.min(activeIndex, Math.max(total - 1, 0))]
    ...
    return (
      <div className="flex aspect-[16/10] items-center justify-center rounded-card border border-dashed border-ink-300 bg-ink-100 text-ink-400">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm">No photos on record</p>
        </div>
      </div>
    )
  }
